...
Sudoku::Sudoku(const vector<vector<int>>& in, const vector<vector<char>>& h, const vector<vector<char>>& v){
    nums = in; horiz = h; vert = v;
}
...
bool vertcomp(Sudoku& s, int x){
    if(x < 0) return true;
    int row = x/9;
    int col = x%9;
    if(row == 8) return true;
    char c = s.vert[row][col];
    int num1 = s.term(x);
    int num2 = s.term(x + 9);
    if(c == '-'){
        if(num2 == 0) return true;
        if(num1 == 0) return true;
        return ((num1 + num2) != 5 && (num1 + num2) != 10);
    }
    if(c == 'x' || c == 'X'){
        if(num2 == 0) return true;
        if(num1 == 0) return true;
    ...
    if(num2 == 0) return num1 < 5;
    if(num1 == 0) return num2 < 5;
    return ((num1 + num2) == 5);
}

bool horizcomp(Sudoku& s, int x){
    if(x < 0) return true;
    int row = x/9;
    int col = x%9;
    if(col == 8) return true;
    char c = s.horiz[row][col];
    int num1 = s.term(x);
    int num2 = s.term(x + 1);
    if(c == '-'){
        if(num2 == 0) return true;
...
bool works(Sudoku& s, int x){
    ...
    if(!vertcomp(s,x) || !horizcomp(s,x) || !vertcomp(s,x - 9) || !horizcomp(s,x - 1)) return false;
    // Works
    return true;
}
...
struct comp{
    bool operator()(pair<int,vector<int>>& lhs, pair<int,vector<int>>& rhs){
        if(lhs.second.size() == rhs.second.size()) return lhs.first < rhs.first;
